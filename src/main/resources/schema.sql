...
-- 3. EARN_TYPE    : ORDER(주문적립), EVENT(이벤트), RE_EARN(취소재적립)
...
CREATE TABLE IF NOT EXISTS user_point_wallet (
    user_point_wallet_id   VARCHAR(50) NOT NULL COMMENT '지갑 식별자 (WLT-...)',
    ...
    PRIMARY KEY (user_point_wallet_id),
...
CREATE TABLE IF NOT EXISTS point_earn_record (
    user_point_wallet_id   VARCHAR(50) NOT NULL COMMENT '지갑 식별자 (FK: WLT-...)',
    ...
    PRIMARY KEY (user_point_wallet_id, earn_id, created_date)
) COMMENT '포인트 적립 이력';
...
CREATE TABLE IF NOT EXISTS point_usage_record (
    ...
    PRIMARY KEY (user_point_wallet_id, usage_id, created_date)
...
CREATE TABLE IF NOT EXISTS point_usage_earn_mapping (
    ...
    PRIMARY KEY (user_point_wallet_id, usage_id, usage_created_date, earn_id)
